...
use optimism_children_indexer_server::Settings;
use reqwest::Url;
...
pub async fn init_optimism_children_indexer_server<F>(db: TestDbGuard, settings_setup: F) -> Url
where
    F: Fn(Settings) -> Settings,
{
    ...
    test_server::init_server(
        || optimism_children_indexer_server::run_server(client, settings),
        &base,
    )
    .await;
    base
