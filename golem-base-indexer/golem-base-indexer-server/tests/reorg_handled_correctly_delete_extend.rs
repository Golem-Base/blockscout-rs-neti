...
async fn test_delete_extend_reorg_handling_works() {
    let db = helpers::init_db("test", "delete_extend_reorg_handling").await;
    ...
    helpers::load_data(
        &*client,
        include_str!("fixtures/reorg_delete_extend_step1.sql"),
    )
    .await;
    ...
    helpers::load_data(
        &*client,
        include_str!("fixtures/reorg_delete_extend_step2.sql"),
    )
    .await;
    indexer.tick().await.unwrap();
    let response: serde_json::Value =
