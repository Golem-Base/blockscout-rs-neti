...
fn endpoint_for_resolution_and_dates(resolution: &str, from: &str, to: &str) -> String {
    format!("/api/v1/chart/data-usage?resolution={resolution}&from={from}&to={to}",)
}
...
#[tokio::test]
#[ignore = "Needs database to run"]
async fn chart_data_usage_should_work_without_data_indexed() {
    // Setup
    let db = helpers::init_db("test", "chart_data_usage_should_work_without_data_indexed").await;
    let client = db.client();
    let base = helpers::init_golem_base_indexer_server(db, |x| x).await;
